...
export class SyncManager {
    ...
    for (const operation of operations) {
      try {
        ...
        if (operation.attempts >= this.config.maxRetryAttempts) {
          this.syncQueue.remove(operation.id);
          console.error(`Operation ${operation.id} failed after ${operation.attempts} attempts:`, errorMessage);
        }
      }
    }
  }
  ...
  private async pullFromServer(): Promise<void> {
    ...
    const localById = new Map(
      localSubscriptions.map(sub => [sub.id, sub])
    );
    ...
    for (const [id] of localById) {
      ...
      const pendingCreate = this.syncQueue.getAll().some(
        op => op.entityId === id && op.entityType === 'subscription'
      );
